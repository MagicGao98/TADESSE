...
public:
  virtual Fitness run(double *fitness_memory) = 0;
protected:
  void evaluatePopulation(const vector<Individual> &pop, vector<Fitness> &fitness);
  void evaluateIndividual(const Individual indiv, Fitness *fit);
  ...
  */
  ...
  /*
    Return random value from normal distribution with mean "mu" and standard deviation "sigma"
...
class TADESSE: public searchAlgorithm {
public:
  virtual Fitness run(double *fitness_memory);
  void setSHADEParameters();
  void reducePopulationWithSort(int reduction_ind_num, vector<Individual> &pop, vector<Fitness> &fitness, vector<int> &stagnation, vector<vector<double>> &dpr);

...
void searchAlgorithm::evaluateIndividual(const Individual indiv, Fitness *fit)
{
  ...
  nfes++;
  cec22_test_func(indiv, fit, problem_size, 1, function_number);
}
...
void searchAlgorithm::modifySolutionWithParentMedium(Individual child, Individual parent) {
  for (int j = 0; j < problem_size; j++) {
    while (child[j] < min_region) {
      child[j]= (min_region + parent[j]) / 2.0;
    }
    while (child[j] > max_region) {
      child[j]= (max_region + parent[j]) / 2.0;
    }
  }
}
